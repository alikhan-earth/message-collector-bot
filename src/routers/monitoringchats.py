    ...
    try:
        ...
        for monitoring_chat in map(check_chat, message.text.split('\n')):
            ...
            if monitoring_chat.strip() in config.monitoring_chats or monitoring_chat in to_append_chats:
                await message.answer(f"""Чат <a href="{'http://t.me/' + monitoring_chat if 'http://t.me' not in monitoring_chat else monitoring_chat}"><b>{monitoring_chat.strip()}</b></a> уже добавлен.""", parse_mode='html')
                continue
...
async def monitoring_chat_list(callback: types.CallbackQuery):
    ...
    for index, monitoring_chat in enumerate(config.monitoring_chats):
        msg += f"""{index+1}. <a href="{'http://t.me/' + monitoring_chat if 'http://t.me' not in monitoring_chat else monitoring_chat}">{monitoring_chat}</a>\n"""
        last_index = index + 1

    for index, to_append_chat in enumerate(to_append_chats):
        msg += f"""{last_index+index+1}. <a href="{'http://t.me/' + monitoring_chat if 'http://t.me' not in monitoring_chat else monitoring_chat}">{to_append_chat}</a> (ожидает добавления)\n"""
    ...
    try:
        ...
        for value in values:
            ...
            if value.strip() not in config.monitoring_chats:
                await message.answer(f"""Чат <a href="{'http://t.me/' + value if 'http://t.me' not in value else value}"><b>{value.strip()}</b></a> отсутствует""", parse_mode='html')
                continue
